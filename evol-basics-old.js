...
var SimEval = component(DIV,"simulation",function(evaluationFunction){
	...
	initState:function(){
		this.dedicatedLab = new MultiVerse();
		//configureInitialState( this.dedicatedLab );
	},
	launch:function(){
		this.initState();
		var el = this._;
		yox(this.dedicatedLab.universes)
...
function configureInitialState(lab){
	...
	lab.universes.forEach(function(realm){
		...
		decoder.d = [];
		var originParticle = realm.createBody(50-1,50,decoder);
		originParticle.tracking = 1;
		repeat(20,function(i){
			realm.createAnchoredRock(i*5+5,75);
		})
	});
...
(function(){
	...
	var launchButton = c(BUTTON,"go","Launch");
	var menu = c(DIV,"menu-bar",[launchButton]);
	simulation.a(menu);
	launchButton.clkOnce(function(){
		configureInitialState(simulation.dedicatedLab);
		simulation.launch();
	});
